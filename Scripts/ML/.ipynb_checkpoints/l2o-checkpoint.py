...
Update: November 15, 2023: Updated for f3dasm 1.4.5
"""
...
RAW_DATASET_LABEL = 'raw'
PERFORMANCE_DATASET_LABEL = 'post'
...
def open_one_dataset_post(experimentdata: ExperimentData,
    ...
    """
    return xr.open_dataset(experimentdata.project_dir /
                           experimentdata.to_pandas()[1]
...
def open_all_datasets_post(experimentdata:
    ...
    datasets at once.
    """
    files = [experimentdata.project_dir / path for path in
             experimentdata.to_pandas()[1]
             [PERFORMANCE_DATASET_LABEL] if
             isinstance(path, str)]

    return xr.open_mfdataset([file for file in files if file.exists()])
...
class WorstPerfProfile(Strategy):
    ...
    def __call__(self, xarr: PerformanceDataset) -> StrategyDataArray:
        return xarr['perf_profile'].median('realization').idxmax(
...
class BestPerfProfile(Strategy):
    ...
    def __call__(self, xarr: PerformanceDataset) -> StrategyDataArray:
        return xarr['perf_profile'].median('realization').idxmin(
            'optimizer', fill_value='RandomSearch').sel(
                output_dim='y').drop('output_dim')
